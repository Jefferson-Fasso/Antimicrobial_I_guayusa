...
``` r
# Para B. subtilis
B_subtilis <- data.frame(readxl::read_excel("Data/Antimicrobial_EthOH.xlsx", 3))

# Para S. aureus
S_aureus <- data.frame(readxl::read_excel("Data/Antimicrobial_EthOH.xlsx", 5))

# Para K. pneumoniae
C_albicans <- data.frame(readxl::read_excel("Data/Antimicrobial_EthOH.xlsx", 6))
```
...
bx_bs <- ggplot(datos_bs,aes(x=extract_bs,y=halo_bs,fill=extract_bs)) +
  geom_boxplot() +
  scale_y_break(c(12, 24), scales = 0.5) +
  ggtitle("B. subtilis") +
...
```
    ...
    ## Bartlett's K-squared = 10.392, df = 3, p-value = 0.01551

Como el p-value es 0.01551, y éste es un valor menor de 0.05, rechazamos
...
``` r
plot(tukey_sa, cex.names=1, ylab = "Diferencia de medias", main="Prueba tukey para S. aureus", font.main=3)
```
...
``` r
plot(tukey_kp, cex.names=1, ylab = "Diferencia de medias", main="Prueba tukey para C. albicans", font.main=3)
```
